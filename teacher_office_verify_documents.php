...
<header class="topbar-nav">
 <nav class="navbar navbar-expand fixed-top">
    ...
    <li class="nav-item">
      <form class="search-bar" action="teacher_office_verify_document_search.php" method ="POST" >
	...
	<form class="search-bar" action="csv_export/verify.php" method ="POST" target ="blank" >
	<li class="nav-item">
       <button type="submit" value = "submit" name="submit" class="btn btn-light btn-block mt-0">
              ...
              <table class="table table-hover">
                <thead>
                  <tr>
						...
						<th scope="col"><center>Comment</th>
                ...
                <?php 
						 ...
						 if ($keyword == ""){
							 $sql = "select * from student_tb where flag = '' or flag = '0' ";
						 }else{
							 $sql = "select * from student_tb where (rollno like '%$keyword%' or caste like '%$keyword%' or first_name like '%$keyword%'or last_name like '%$keyword%') and flag = 0 order by rollno asc ";
						 }
											...
											while($row = mysqli_fetch_assoc($result)) {
												...
												." </td><td><center>".$row['comment'] 
												." </td><td><center>"."<form action = 'teacher_office_reject_stud.php' method = 'POST' target=''>
												<input type='hidden' value='$row[rollno]' name = 'roll'>												
												<input type='hidden' value='Reject' name = 'val'>
												<input type='text' class='' placeholder='Reason for Rejection' name = 'comment_text'>
												<button type ='submit' class='btn btn-primary' value='submit'>Reject</button></form>"
												." </td><td><center>"."<form action = 'teacher_office_reject_stud.php' method = 'POST' target=''>
												<input type='hidden' value='$row[rollno]' name = 'roll'>												
												<input type='hidden' value='Accept' name = 'val'>
												<input type='hidden' class=''  name = 'comment_text'>
												<button type ='submit' class='btn btn-primary' value='submit]'>Accept</button></form>"
												."</a></tr></td>";
